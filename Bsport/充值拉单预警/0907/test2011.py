...
data_ori = {
    'dateType':'1',
    'startTime':int(time.time()-(30*60))*1000,
...
def job():
    ...
    result2.drop_duplicates('账户名',inplace=True)
    result2.set_index('账户名',inplace=True)
    result2 = result2.loc[result2['数量']>3,:]
    df2.drop_duplicates('账户名',inplace=True)
    ...
    for page in range(1, 16):
        data_huiyuan = {
            ...
            'searchType': '1',
            'channelId': '34',
            'registeredStartDate': int(today_1.timestamp()) * 1000,
    ...
    for user in result2.index:
        if user in ori_dic:
            if int(time.time())-ori_dic[user] >3600:

                fw.write(f'会员等级：{df2.loc[user,"VIP等级"]}\n')
                fw.write(f'账号：{" "}{user}\n')
                ...
                fw.write(f'半小时内申请次数：{result2.loc[user,"数量"]}\n')
                fw.write('-----------------------\n')
    ...
    if sum_>0:
        ...
        # bot_da.send_message(-677235937,text)   充值拉单： -996504819
        bot_da.send_message(-677235937,text,timeout=500)
        #bot_da.send_message(-677235937, text, timeout=500)
        bot_da.stop_polling()
    else:
        bot_da = telebot.TeleBot('6106076754:AAHjxPSBpyjwpY-lq1iEslUufW46XQvAfr0')
        bot_da.send_message(6279115720,'---------充值拉单1.3.2',timeout=500)
        bot_da.stop_polling()

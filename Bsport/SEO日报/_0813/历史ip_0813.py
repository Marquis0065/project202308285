...
data_2 = pd.read_excel(r'C:\Users\User\Desktop\SEO\_0813\今日数据.xlsx','趋势分析')
daili = pd.read_excel(r'C:\Users\User\Desktop\SEO\_0813\SEO每日模板-每日更新.xlsx','代理总表')
...
for h in hour_list:
    ...
    print(h,df.loc[(df['人员']=='Zed') & (df['指标']=='发送IP数'),h])
    ...
    print(h,df.loc[(df['人员']=='Zed') & (df['指标']=='接收IP数'),h])
    print('接收率：',round(df.loc[(df['人员']=='Zed') & (df['指标']=='接收IP数'),h].iloc[0] / df.loc[(df['人员']=='Zed') & (df['指标']=='发送IP数'),h].iloc[0]*100,2)
          )
    ...
    print(df.loc[(df['人员']=='Zed') & (df['指标']=='接收率'),h])
...
#-----------------------------------Aber----------
for h in hour_list:
    df.loc[(df['人员']=='Aber') & (df['指标']=='发送IP数'),h] =data_2[data_2['网站名(domain)'].str.contains('aber.com')&(data_2['时间']>=int(h.split('-')[0])) & (data_2['时间']<int(h.split('-')[1]))]['IP'].sum() // 2

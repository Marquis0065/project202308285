...
def huiyuan_q_fun(start_page,end_page):
    ...
    return  pd.DataFrame(dic_huiyuan)

huiyuan1 = pd.DataFrame(columns=['会员账号','代理','电话','vip等级','注册时间','注册地'])
huiyuan2 = pd.DataFrame(columns=['会员账号','代理','电话','vip等级','注册时间','注册地'])
for page in range(len(page_list)):
    df = huiyuan_q_fun(page_list[page]+1,page_list[page+1])
    if page<=3:
        huiyuan1= huiyuan1.append(df)
        print('当前数据：',huiyuan1.shape)
        print('等待5分钟后继续。。')
        time.sleep(350)
    else:
        huiyuan2= huiyuan2.append(df)
        print('当前数据：',huiyuan2.shape)
        print('等待5分钟后继续。。')
        time.sleep(350)

print('huiyuan1:',huiyuan1.shape)
print('huiyuan2:',huiyuan2.shape)

write = pd.ExcelWriter(r'C:\Users\User\Desktop\文件\定时任务\会员列表更新\会员总表.xlsx')
huiyuan1.to_excel(write,sheet_name='1表')
huiyuan2.to_excel(write,sheet_name='2表')
write.save()
write.close()
